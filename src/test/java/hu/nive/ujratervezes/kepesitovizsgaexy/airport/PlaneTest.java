...
class PlaneTest {
    ...
    void relocate() {
        ...
        plane.relocate();
        List<Person> result = plane.getPeople();
    ...
    void findTheCaptain1() {
        assertEquals("Scott", new Plane(people).findTheCaptain().getName());
    }

    @Test
    void findTheCaptain2() {
        Exception exception = assertThrows(IllegalStateException.class, () -> new Plane(List.of()).findTheCaptain());
        assertEquals("The plane is empty!", exception.getMessage());
    }
    ...
    void howOldTheCaptain() {
        assertEquals(39, new Plane(people).howOldTheCaptain().get());
        assertEquals(Optional.empty(), new Plane(List.of()).howOldTheCaptain());
    }
}